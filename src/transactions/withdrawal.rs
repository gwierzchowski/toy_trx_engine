...
mod tests {
    ...
    fn on_locked() {
        ...
        let trx = Withdrawal {client: 1, tx: 1, amount: 1.0};
    ...
    fn on_normal() {
        let mut accounts = create_accounts();
        let trx = Withdrawal {client: 2, tx: 1, amount: 1.0};
    ...
    fn unknown_client() {
        let mut accounts = create_accounts();
        let trx = Withdrawal {client: 20, tx: 1, amount: 1.0};
    ...
    fn over_balance() {
        let mut accounts = create_accounts();
        let mut trx = Withdrawal {client: 2, tx: 1, amount: 1.0};
        let old_balance = accounts.get(&trx.client).expect("client 2 in test accounts").available;
        trx.amount = old_balance + 0.1;
        assert!(trx.commit(&mut accounts).is_err());
    }
    ...
    fn duplicated_tx_id() {
        let mut accounts = create_accounts();
        let trx1 = Withdrawal {client: 2, tx: 1, amount: 0.1};
        assert!(trx1.commit(&mut accounts).is_ok());
        let trx2 = Withdrawal {client: 2, tx: 1, amount: 0.1};
        assert!(trx2.commit(&mut accounts).is_err()); // duplicated id
        let trx3 = Withdrawal {client: 2, tx: 2, amount: 0.1};
